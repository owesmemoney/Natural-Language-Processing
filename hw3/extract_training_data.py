...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        #initiate ; top of stack is the last word in the list; next word in buffer the last word in the list
        input_rep = []
        stacks = state.stack
        buffer = state.buffer
        input_pos = {value:pos[i] for i,value in enumerate(words)}# assembly of pos for all input words
            
        # test stack;next three word on stack
        for i in range(-1,-4,-1):# for -1, -2, -3
            try:
                s = words[stacks[i]]
                
                if input_pos[s] == None:
                    #special root symbol
                    input_rep.append(self.word_vocab["<ROOT>"])
                elif input_pos[s] == 'CD':
                    #any number
                    input_rep.append(self.word_vocab['<CD>'])
                elif input_pos[s] == 'NNP':
                    #any proper name
                    input_rep.append(self.word_vocab["<NNP>"])
                else:
                    
                    if s.lower() in self.word_vocab:
                        input_rep.append(self.word_vocab[s.lower()])
                        #words does not appear in words.vocab
                    else:
                        input_rep.append(self.word_vocab["<UNK>"])
            except:#pad context windows
                input_rep.append(self.word_vocab["<NULL>"])
            
         # test buffer; top-three words on buffer
        for i in range(-1,-4,-1):
            try:
                b = words[buffer[i]]
                if input_pos[b] == None:
                    #initially placed on the dependency parser
                    input_rep.append(self.word_vocab["<ROOT>"])
                elif input_pos[b] == 'CD':
                    #anything taged with POS tag CD
                    input_rep.append(self.word_vocab['<CD>'])
                elif input_pos[b] == 'NNP':
                    #anything tagged with POS tag NNO
                    input_rep.append(self.word_vocab["<NNP>"])
                else:
                    
                    if b.lower() in self.word_vocab:
                        input_rep.append(self.word_vocab[b.lower()])
                        #check the same way for buffer.words not in words.vocab belongs to UNK
                    else:
                        input_rep.append(self.word_vocab["<UNK>"])
            except:
                #the same as stack
        ...
        return np.array(input_rep)

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        output_rep = self.output_labels[output_pair]
        return tf.keras.utils.to_categorical(output_rep, num_classes=91, dtype="float32")
