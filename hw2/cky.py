...
class CkyParser(object):
    ...
    def is_in_language(self,tokens):
        ...
        return False
        check if the top position is TOP using CKY full algorithm in slide 7 p10
        """
        # TODO, part 2
        #initialization: create a n*n chart and initialize it
        n = len(tokens)
        ...
        for i in range(n):
                for j in self.grammar.rhs_to_rules[(tokens[i],)]: 
                    chart[i][i].append(j[0])
            #pie[i,i+1]={A| A- s[i]}
        #main loop; fill in the chart
        ...
        for length in range(2, n + 1):
            for i in range(n - length + 1):
                ...
                for k in range(i + 1, j):
                    for rule in rules:
                        if chart[i][k] and chart[k][j]:
                            for item in chart[i][k]:
                                for item_ in chart[k][j]:
                                    if rule[0] == item and rule[1] == item_:
                                        for x in grammar.rhs_to_rules[rule]:
                                            chart[i][j].append(x[0])
        if chart[0][n-1] != []:
            return True
        else:
            return False
